...
def NonUnif_Quantization(weights,Nbit):
    ...
    return torch.nn.Parameter(torch.Tensor.double(torch.from_numpy(Quantized_weight)).to(dtype=torch.float32)), MSE
...
def RunExp(args, dataset, data, Net, percls_trn, val_lb):
    ...
    Led=['Original Gamma']
    data_save = np.load("quan_non_uni.npz", allow_pickle=True)
    ...
    for Nbit in range(9):
        ...
        model_quantized.lin1.weight.data, MSE_w1=NonUnif_Quantization(model_quantized.lin1.weight.data,Nbit)
        model_quantized.lin1.bias.data, MSE_b1=NonUnif_Quantization(model_quantized.lin1.bias.data,Nbit)
        model_quantized.lin2.weight, MSE_w2=NonUnif_Quantization(model_quantized.lin2.weight,Nbit)
        model_quantized.lin2.bias.data, MSE_b2=NonUnif_Quantization(model_quantized.lin2.bias.data,Nbit)
    ...
    np.savez("quan_non_uni", data=results)
